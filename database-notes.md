...
* Scales vertically
* **Reads** can be very optimised with indexes and the relationships know in advance however foreign keys and joins can degrade this quickly
**Write** performance generally suffers due to ACID
...
|Type|Description|
|---|---|
|Vertical|By feature/domain objects. Usually a table is a domain object representation, instead, this will be a database.<br/> **Cons**: If the database grows too big, another partitioning scheme may need to be applied.|
|Key-based (hashing)|Could be split by an ID/value and that will be used for the server name/number. For example, if you have `n` servers, `mod(key, n)` will tell you where the data is stored.<br/> **Cons**: `n` is effectively fixed, if it changes, then the data migrations could be costly.|
|Directory-based|Use a lookup table to determine where the data is stored.<br/> **Cons**: single point of failure and bottleneck, costly lookup at scale.|
...
* Useful for columns that are frequently used in **range** (e.g. `BETWEEN` or `IN`) queries or when retrieving large result sets or when relying on the *sorted values*.
